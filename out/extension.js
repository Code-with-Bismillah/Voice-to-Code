...
function activate(context) {
    ...
    const insertCodeCommand = vscode.commands.registerCommand("voice-to-code.insertCode", (code) => {
    ...
    const transcribeCurrentMediaCommand = vscode.commands.registerCommand("voice-to-code.transcribeCurrentMedia", async () => {
    ...
    context.subscriptions.push(startListeningCommand, stopListeningCommand, toggleListeningCommand, insertCodeCommand, createNewFileCommand, checkPythonCommand, transcribeMediaCommand, transcribeCurrentMediaCommand, installDependenciesCommand, testMicrophoneCommand, statusBarItem);
}
...
- Ensure internet connection for Google Speech API
  `;
    const action = await vscode.window.showInformationMessage("Microphone access is required for voice recognition. Check the help for setup instructions.", "Show Help", "Test Now", "Open Settings");
    if (action === "Show Help") {
...
async function installPythonDependencies() {
    try {
        ...
        const action = await vscode.window.showInformationMessage("This will install required Python packages: SpeechRecognition, pydub, moviepy", "Install Now", "Cancel");
...
async function transcribeMediaFile(context) {
    try {
...
async function transcribeCurrentMediaFile(context) {
    ...
    if (!mediaExtensions.includes(fileExtension)) {
        vscode.window.showErrorMessage("Current file is not a media file. Please open a media file or use 'Transcribe Media File' command.");
        return;
    }
    await performMediaTranscription(context, currentFilePath);
}
async function performMediaTranscription(context, mediaFilePath) {
    try {
        const pythonPath = await findPythonExecutable();
        const config = vscode.workspace.getConfiguration("voice-to-code");
        ...
        const fileType = path.extname(mediaFilePath).toLowerCase().includes("mp4") ||
        ...
        const transcriptionProcess = (0, child_process_1.spawn)(pythonPath, [pythonScriptPath, mediaFilePath, language, useChunkMode.toString()], {
        ...
        transcriptionProcess.stderr?.on("data", (data) => {
            ...
            if (error.includes("Missing dependencies")) {
                vscode.window
                    .showErrorMessage("Required Python modules not found. Click 'Install Dependencies' to install them automatically.", "Install Dependencies", "Manual Install")
                    .then((action) => {
...
async function handleTranscriptionResult(transcriptionText, mediaFilePath) {
    const config = vscode.workspace.getConfiguration("voice-to-code");
    ...
    if (autoInsert) {
        ...
        await vscode.window.showTextDocument(doc);
        vscode.window.showInformationMessage(`Media transcribed successfully! Generated ${processedCode.length} characters of code.`);
    }
    else {
        // Show preview
        const action = await vscode.window.showInformationMessage(`Transcription: "${transcriptionText}"\nGenerated code: ${processedCode}`, "Insert Code", "Create New File", "Copy to Clipboard");
...
async function startListening(context) {
    ...
    if (!vscode.window.activeTextEditor) {
        const action = await vscode.window.showInformationMessage("No active editor found. Would you like to create a new file?", "Create New File", "Cancel");
    ...
    try {
        ...
        pythonProcess = (0, child_process_1.spawn)(pythonPath, [scriptToUse, language], {
            cwd: path.join(context.extensionPath, "python"),
        });
        isListening = true;
        updateStatusBar();
        ...
        pythonProcess.stderr?.on("data", (data) => {
            ...
            if (error.includes("ModuleNotFoundError")) {
                vscode.window
                    .showErrorMessage("Required Python modules not found. Click 'Install Dependencies' to install them automatically.", "Install Dependencies", "Help")
                    .then((action) => {
                    if (action === "Install Dependencies") {
                        vscode.commands.executeCommand("voice-to-code.installDependencies");
                    }
                ...
                vscode.window
                    .showErrorMessage("Microphone access failed. Please check microphone permissions and try again.", "Help", "Test Microphone")
                    .then((action) => {
                    if (action === "Help") {
...
async function handleVoiceInput(voiceText) {
    const config = vscode.workspace.getConfiguration("voice-to-code");
    const autoInsert = config.get("autoInsert", true);
    const showPreview = config.get("showPreview", true);
    // Process voice text to code
    const processedCode = processVoiceToCode(voiceText);
    if (showPreview && !autoInsert) {
        const action = await vscode.window.showInformationMessage(`Recognized: "${voiceText}"\nGenerated code: ${processedCode}`, "Insert Code", "Dismiss");
        if (action === "Insert Code") {
            insertCodeIntoEditor(processedCode);
        }
    }
...
function processVoiceToCode(voiceText) {
...
async function checkPythonAvailability(pythonPath) {
    return new Promise((resolve, reject) => {
        const pythonCheck = (0, child_process_1.spawn)(pythonPath, ["--version"]);
